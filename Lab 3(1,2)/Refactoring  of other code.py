...
def triangle(a):
    '''
    This function draws triangles around the sun
    a is parametr of extra turn
    '''
...
def cloud(x, y, e, k):
    ...
    x, y are coordinates of the left top angle of left top cloud 
    e is excentricitet
    k is retative size. Absolute height is 25'''
    r = 25 * k      #this is height of the ellipse
    g = 0.8         #this is  x displacement factor
    for i in range(4):       #draw upper clouds
        ellipse(screen, (255, 255, 255), [x + g * r * i, y - r, e * r, r], 0)    #draw white cloud
        ellipse(screen, (0, 0, 0), [x + g * r * i, y - r, e * r, r], 1)          #draw black contur
    for i in range(3):        #draw lower clouds
        ellipse(screen, (255, 255, 255), [x + g * r * (i + 1 / 2), y - r / 2, e * r, r], 0)    #draw white cloud
        ellipse(screen, (0, 0, 0), [x + g * r * (i + 1 / 2), y - r / 2, e * r, r], 1)          #draw black contur
    return None

# Umbrella
def umbrella(x, y, k): 
    '''
    Function drawing an umbrella
    x, y - coordinates of left bottom corner of stick, k - scale
    '''
...
def ship(x, y, k):
    '''
    Function drawing a ship
    x, y are coordinates of left bottom corner of the ship
    k is scale'''
...
#draw ships, clouds and umbrellas
ship(130, 210, 0.6)
ship(350, 230, 1)
